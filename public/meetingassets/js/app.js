...
//first we write thids code for cilent side connection socket
var MyApp = (function () {
  var socket = null;
  var user_id = "";
  var meeting_id = "";
  ...
  function event_process_for_signaling_server() {
    socket = io.connect();//socket io connect with serve.js
    ...
    socket.on("connect", () => {//here socket io connect with client side
      if (socket.connected) {
        AppProcess.init(SDP_function, socket.id);
        if (user_id != "" && meeting_id != "") {
          socket.emit("askToConnect", {//emit means send to socket
            displayName: user_id,
  ...
  //here we give meeting id and user id script
  return {
